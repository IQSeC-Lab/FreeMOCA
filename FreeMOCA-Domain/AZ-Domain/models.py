...
#         """
#         Args:
#             input_features (int): Dimension of input features (EMBER default: 2381)
#             output_dim (int): Number of classes (Fixed for Domain-IL, e.g., 100)
...
#         # We treat feature vector as a signal with Channels=Features, Length=1? 
#         # Or more likely for EMBER: Features are spatial.
#         # Your original code forces a view: x.view(batch, input_features, -1)
#         # implying you treat the feature vector as having 'input_features' channels and length 1?
#         # Actually, looking at kernel_size=3, stride=3 on input 2381...
#         # It seems you treat the 2381 features as a 1D sequence. 
#         # Let's align with your original view logic: (Batch, Features, 1) -> Conv1d.
        
#         self.block1 = nn.Sequential(
#             nn.Conv1d(in_channels=self.input_features, out_channels=512, kernel_size=3, stride=3, padding=1),
#             nn.BatchNorm1d(512),
#             nn.ReLU(),
...
#         self.fc1_bn1 = nn.BatchNorm1d(self.output_dim)
#         self.fc1_drop1 = nn.Dropout(self.drop_prob)
#         # Note: Usually ReLU is NOT applied after the final classification layer logits, 
#         # but your original code had it. I will keep it if that's your specific design choice,
#         # otherwise, standard practice is to remove activation on the very last output.
#         # self.fc1_act1 = nn.ReLU() 
...
#         Returns Raw Logits.
#         """
#         # Save original shape info for reconstruction if needed
#         original_shape = x.size()

#         # 1. Flatten Batch & Time dimensions if input is 3D [Batch, Time, Features]
#         if x.dim() == 3:
#             # Collapse Batch and Time: [B, T, F] -> [B*T, F]
...
#         # 2. Reshape for Conv1d: [Batch, Input_Features, 1]
#         # This treats every feature as a channel with length 1? 
#         # CAUTION: Your original code did: x.view(batch, self.input_features, -1)
#         # If your data is a flat vector of size 2381, this makes it [Batch, 2381, 1].
#         # Then Conv1d kernel=3 runs over the '1' dimension? That requires padding to work.
#         # Or did you mean x.view(batch, 1, self.input_features)?
#         # Assuming your original logic works for your data shape:
#         x = x.view(batch_size, self.input_features, -1) 
...
#         # Removed fc1_act1 (ReLU) on final logits to allow negative values (standard for Logits)
#         # Removed Softmax to allow CrossEntropyLoss to work correctly
        
#         # 5. Restore Time dimension if necessary
#         if len(original_shape) == 3:
#             x = x.view(original_shape[0], original_shape[1], -1)
...
#         """
#         Helper for inference when you actually need probabilities
#         """
...
#     def expand_output_layer(self, init_classes, nb_inc, task):
#         """
#         Retained for Class-IL compatibility. 
#         NOT used in Domain-IL.
#         """
#         old_fc1 = self.fc1
#         old_fc1_bn1 = self.fc1_bn1
#         self.output_dim = init_classes + nb_inc * task
...
#     def __init__(self, input_features=2439, init_classes=50, drop_prob=0.5):
#         super(Classifier, self).__init__()
...
# ... [Keep your Classifier class here] ...

...
import subprocess
import random
from models import Classifier, interpolate_models, Domain_Classifier
...
for task in range(config.nb_task):
    ...
    # FIX the number of classes for all tasks
    config.n_class = config.init_classes

    config.task = task

    # Load data
    X_train_t, Y_train_t, train_loader, X_test_t, Y_test_t, test_loader, scaler = data_task_domain(config, X_train, Y_train, X_test, Y_test, scaler)
    config.nb_batch = int(len(X_train_t) / config.batchsize)
    C.to(device)

    # only for class incremental, should be commented for domain incremental learning
    # if task > 0:

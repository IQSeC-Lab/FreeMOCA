...
#     curves = []
#     for f in files:
#         # parse t from filename curve_t_to_t1.npz
#         base = os.path.basename(f).replace(".npz", "")
#         parts = base.split("_")
#         t = int(parts[1])  # "curve", t, "to", t1
#         data = np.load(f)
...
#     # assume all curves have same s grid
#     s = curves[0]["s"]
#     T = len(curves)

#     # X = transition index (0..T-1), Y = s (0..1)
#     X = np.arange(T)
#     Y = s
#     XX, YY = np.meshgrid(X, Y, indexing="xy")

#     Z = np.zeros_like(YY, dtype=float)  # shape [len(s), T]

#     for j, c in enumerate(curves):
#         loss = c["loss"].copy()
#         if use_relative:
#             # relative-to-endpoints baseline (shows "barrier" cleanly)
...
#     ax.plot_surface(XX, YY, Z, rstride=1, cstride=1, linewidth=0, antialiased=True, alpha=0.95)
...
#     # Mark barrier peaks (max along s for each transition)
#     for j, c in enumerate(curves):
#         s = c["s"]
#         loss = c["loss"].copy()
#         if use_relative:
...
#         ax.scatter(j, s[k], loss[k], marker="x", s=40)
...
#     plot_surface(curves,
#                  title="FreeMOCA: chain of connected valleys (final→final)", use_relative=True)

...
def get_domain_acc_data(data_root, tasks, j):
    ...
    filename = "XY_test.npz"
    X_list = []
    Y_list = []

    for k in range(j + 1):
        domain_folder = tasks[k]
        folder_path = os.path.join(data_root, domain_folder)
...
def get_domain_data(data_root, domain_folder, is_train=True):
    """
    Loads data from: data_root/domain_folder/XY_train.npz
    Example: /data1/.../EMBER_Domain/2018-01/XY_train.npz
    """
    
    # 1. Construct Path
    folder_path = os.path.join(data_root, domain_folder)
    filename = 'XY_train.npz' if is_train else 'XY_test.npz'
    file_path = os.path.join(folder_path, filename)
    ...
    try:
        ...
        print(f"[ERROR] Failed to load keys from {filename}: {e}")

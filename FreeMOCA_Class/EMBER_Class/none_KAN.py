...
def main():
    ...
    # Optimizer will be defined task by task
    ...
    for task in range(config.nb_task):
        ...
        for epoch in tqdm(range(config.epochs), desc=f"task, {task}"):
            C.train()
        ...
        if task > 0:
            C = interpolate_models_KAN(past_Classifier, current_Classifier, alpha=0.7, method="spline")
            C.to(device)
        ...
        with torch.no_grad():
            ...
            # wandb.log({f"accuracy/task_{task}": accuracy})
            ls_a.append(accuracy)
        print(f"task {task} done, accuracy={accuracy}")

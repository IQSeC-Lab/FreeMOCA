...
def get_dataloader(x, y, batchsize, n_class, scaler, train = True):
    ...
    if train: Loader = torch.utils.data.DataLoader(data_tensored, batch_size=batchsize, num_workers=1, sampler=sampler, drop_last=True)
...
def get_iter_test_dataset_task(x, y, init_classes, n_inc, task_id):
    ...
    # print(f"get_iter_test_dataset_task, task_id: {task_id}, start: {start} end: {end}")

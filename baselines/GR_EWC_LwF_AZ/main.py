# https://github.com/msrocean/continual-learning-malware
...
from train import train_iter_cl, train_epoch_cl
...
task_params.add_argument('--tasks', type=int, default='11', help='number of tasks')
#MARK: number of classes
...
task_params.add_argument('--data_set', type=str, choices=['drebin', 'EMBER', 'ANDROZOO'], help='dataset to use')
...
train_params.add_argument('--batch', type=int, default=256, help="batch-size") # default = 256

#MARK: epoch
train_params.add_argument('--epoch', type=int, default=50, help="number of training epochs")
...
def run(args, verbose=True):
    ...
    if args.data_set == 'EMBER':
        orig_feats_length, target_feats_length =  2381, 2401
        num_training_samples = 303331
        args.batch = 512
        args.lr = 0.001
        args.optimizer = 'sgd'
        results_save_dir = './ember_results/' + str(args.scenario) + '/'
        create_parent_folder(results_save_dir)

    elif args.data_set == 'ANDROZOO':
        orig_feats_length, target_feats_length = 2439, 2500
        num_training_samples = 257023
        args.batch = 512
        args.lr = 0.001
        args.optimizer = 'sgd'
        results_save_dir = './androzoo_results/' + str(args.scenario) + '/'
        create_parent_folder(results_save_dir)
    ...
    num_training_samples, (train_datasets, test_datasets), config, classes_per_task, test_set = get_malware_multitask_experiment(
        dataset_name=args.data_set, target_classes=target_classes, init_classes=init_classes,\
        scenario=scenario, orig_feats_length=orig_feats_length,\
        target_feats_length=target_feats_length, tasks=args.tasks, data_dir=args.d_dir,
        verbose=verbose)
    
    #MARK: iteration
    #num_training_samples = 303331
    args.iters = int(np.ceil(num_training_samples/args.batch) * args.epoch)
    # args.iters = 2 # for testing
    # args.g_iters = args.iters #if args.g_iters is None else args.g_iters
    print(f"Number of iterations: {args.iters}")
    #req_iterations = args.iters
    #args.giters = int(np.ceil(num_training_samples/args.batch) * args.epoch)
    
    #print(train_datasets)
    #print(test_datasets)
    
    #for i in range(len(train_datasets)):
    #    print(train_datasets[i], test_datasets[i])
    ...
    if args.pdf or args.metrics:
        ...
        if not args.use_exemplars:
            metrics_dict = evaluate.intial_accuracy(args, model, test_datasets, metrics_dict,
                                                    ...
                                                    test_size=30000, no_task_mask=False)
    else:
        metrics_dict = None
    ...
    print("\nbefore train\n")
    train_iter_cl(
        model, model_save_path, train_datasets, test_datasets, args, replay_mode=args.replay, scenario=scenario,
        ...
        metric_cbs=metric_cbs, use_exemplars=args.use_exemplars, add_exemplars=args.add_exemplars, test_set=test_set
    )
    ...
    # scenario=task 이면 평가 시 allowed_classes가 task별로 제한됨
    ...
    #average_f1score = sum(f1socre) / args.tasks
    ...
    if verbose:
        ...
        for i in range(args.tasks):
            print(" - Task {}: {:.5f}".format(i + 1, precs[i]))
        print('=> Average precision over all {} tasks: {:.5f}\n'.format(args.tasks, average_precs))
    ...
    if args.metrics:
        ...
        if args.scenario in ('task', 'domain'):
            ...
            # 평가하는 task의 클래스만 고려
            # FWT, BWT, F 계산에 사용되는 metric
            R = pd.DataFrame(data=metrics_dict['acc per task (all classes up to evaluated task)'],
                             index=['after task {}'.format(i + 1) for i in range(args.tasks)])
            print("Accuracy matrix we will use for the BWT and FWT")
            copy_R = R.copy()
            print(copy_R.to_string())
            copy_R = copy_R.values
            R.loc['at start'] = metrics_dict[
                'initial acc per task (only classes in task)'
            ] if not args.use_exemplars else ['NA' for _ in range(args.tasks)]
            R = R.reindex(['at start'] + ['after task {}'.format(i + 1) for i in range(args.tasks)])
            ...
            FWT = compute_fwt(copy_R)
            BWT = compute_bwt(copy_R)
            F = compute_forgetting(copy_R)
            REM, BWT_plus = compute_rem_and_bwt_plus(BWT)
            metrics_dict['BWT'] = BWT
            metrics_dict['F'] = F
            metrics_dict['FWT'] = FWT
            metrics_dict['REM'] = REM
            metrics_dict['BWT+'] = BWT_plus
            # -print on screen
            if verbose:
                print("Accuracy matrix")
                print(R.to_string())
    ...
    # 실험 이름(exp_name)과 해당 실험에서 얻은 평균 정확도 추이(metrics_average 리스트) 를 기록
    print(metrics_dict['average'], np.mean(metrics_dict['average']))
    
    metrics_average = metrics_dict['average']
    exp_name = args.exp_name
    results_save_dir = 'ember_results_cl/classes_' + str(target_classes) + '/'
    create_parent_folder(results_save_dir)
    ...
    result_string = '\n\nEXP NAME\t\t{}\t\t\nMetrics Dict\t\t{}\t\n'.format(exp_name, metrics_average)

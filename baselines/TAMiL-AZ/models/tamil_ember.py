...
class TAMIL_Ember(nn.Module):
    ...
    def forward(self, x, task_id):
        ...
        # TAMIL core: 현재 task의 AE를 통과시켜 latent embedding 생성
        feats = self.backbone(x)
        ae_out = self.task_aes[task_id](feats)
        # 특징 * AE 결과(elementwise product): TAMIL 논문 방식
    ...
    def observe(self, x, y, bx=None, by=None, buffer_weight=1.0):
        ...
        if bx is not None and by is not None:
            buffer_logits = self.forward(bx, self.current_task)  # 또는 bx의 task info에 따른 태스크 지정!

...
def main():
  ...
  for task in range(config.nb_task):
    ...
    config.task = task
    X_train_t, Y_train_t, X_test_acc_t, Y_test_acc_t, X_test_bwt_t, Y_test_bwt_t, train_loader, acc_test_loader, bwt_test_loader = data_task(config, X_train, Y_train, X_test, Y_test)
  ...
  rows = ([f"after task {i}" for i in range(1, 12)])

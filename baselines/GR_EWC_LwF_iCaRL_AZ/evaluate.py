...
def metric_statistics(args, model, datasets, current_task, iteration, classes_per_task=None, scenario="domain",
    ...
    for i in range(n_tasks):
        ...
        #MARK: acc 계산 코드
        #MARK: all classes up to trained task
        # -all classes up to trained task
        if scenario in ('class'): # 여기 사용
            #allowed_classes = list(range(classes_per_task * current_task))
        ...
        #MARK: BWT, FWT 계산 코드
        #MARK: all classes up to evaluated task
        ...
        if scenario in ('class'):
            ...
            if (not with_exemplars):
                precision = validate(model, datasets[i], test_size=test_size, verbose=verbose,
                                 allowed_classes=allowed_classes, no_task_mask=no_task_mask, task=i + 1,
                                 with_exemplars=with_exemplars) # if (not with_exemplars) else 0. # or (i<current_task) else 0.
            else:
                if (i<current_task):
                    precision = validate(model, datasets[i], test_size=test_size, verbose=verbose,
                                     allowed_classes=allowed_classes, no_task_mask=no_task_mask, task=i + 1,
        ...
        #MARK: only classes in that task
        # -only classes in that task
        if scenario in ('task', 'class'):
            if scenario == 'class': # 여기 사용
                if i == 0:
                    allowed_classes = list(range(init_classes))
                else:
                    allowed_classes = list(range(init_classes + classes_per_task * (i-1), init_classes + classes_per_task * (i)))
    ...
    elif scenario=='class': # 여기 사용
        # 수정
        total_correct = 0
        samples = 0
        for task_id in range(current_task):
            num_samples = len(datasets[task_id][1])
            accuracy = precs_all_classes_so_far[task_id]
            total_correct += accuracy * num_samples
            samples += num_samples
        average_precs = total_correct / samples
        print(f"num_samples per task: {[len(datasets[task_id][1]) for task_id in range(current_task)]}")
        print(f"number of samples so far: {samples}")
        print(f"average_precs: {average_precs}")
...
def oh(Y, num_classes):
    ...
    Y_oh = torch.nn.functional.one_hot(Y.to(torch.int64), num_classes=num_classes)
    return Y_oh

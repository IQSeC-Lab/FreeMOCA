...
class Classifier(ContinualLearner, Replayer, ExemplarHandler):
    ...
    def __init__(self, image_size, image_channels, classes,
        ...
        sizes_layers = [2500, 1024, 512, 256, 128] # 2401 -> 2500
        
        #[1024, 512, 256, 128]
        # fully connected hidden layers
        
        # 2401 -> 2500
        self.fcE = MLP(input_size=2500, output_size=fc_units, layers=fc_layers-1,
    ...
    def train_a_batch(self, x, y, scores=None, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, task=1):
        ...
        if x is not None:
            ...
            if self.binaryCE:
                C = y_hat.size(1)
                binary_targets = utils.to_one_hot(y.cpu(), C).to(y.device)
                if self.binaryCE_distill and (scores is not None):
                    # scores: 이전 모델의 logits for OLD classes
                    n_old = scores.size(1)          # 예: 50
                    n_new = C - n_old               # 예: 5
                    distil_part = torch.sigmoid(scores / self.KD_temp)
                    new_part = binary_targets[:, n_old:]
                    binary_targets = torch.cat([distil_part, new_part], dim=1)

...
class DataManager(object):
    ...
    def get_dataset(
        ...
        if ret_data:
            return data, targets, DummyDataset(data, targets, trsf, self.use_path, self.dataset_name)
        else:
            return DummyDataset(data, targets, trsf, self.use_path, self.dataset_name)
    ...
    def _setup_data(self, dataset_name, shuffle, seed):
        ...
        if dataset_name=="ember":
            X_train = np.load('../data/EMBER_Class/X_train.npy')
            Y_train = np.load('../data/EMBER_Class/Y_train.npy')
            X_test = np.load('../data/EMBER_Class/X_test.npy')
            Y_test = np.load('../data/EMBER_Class/Y_test.npy')
            scaler = StandardScaler()
            X_train = scaler.fit_transform(X_train)
            X_test = scaler.transform(X_test)
            X_train = X_train.astype(np.float32)
            X_test = X_test.astype(np.float32)
        elif dataset_name=="az":
            tr_data = np.load('../data/AZ_Class/AZ_Class_Train.npz')
            X_train = tr_data['X_train']
            Y_train = tr_data['Y_train']
            te_data = np.load('../data/AZ_Class/AZ_Class_Test.npz')
            X_test = te_data['X_test']
            Y_test = te_data['Y_test']
        
        # Data
        self._train_data, self._train_targets = X_train, Y_train
        self._test_data, self._test_targets = X_test, Y_test
        self.use_path = False
...
class DummyDataset(Dataset):
    def __init__(self, images, labels, trsf, use_path=False, sub_dataset=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.sub_dataset == "ember":
            if x_tensor.shape[0] < 2401:
                ...
                x_tensor = torch.cat(
                    [x_tensor, torch.zeros(pad_len, dtype=torch.float32)], dim=0
                )

            x_tensor = x_tensor.view(49, 49)
            x_tensor = x_tensor.unsqueeze(0)    # (1,49,49)
        ...
        # AZ
            ...
            x_tensor = x_tensor.unsqueeze(0)    # (1,50,50)

        return idx, x_tensor, y

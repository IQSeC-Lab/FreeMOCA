...
init_epoch      = 1 # _init_train에 있음, 200 이 오리지널
init_lr         = 0.1
init_milestones = [60, 120, 170]
init_lr_decay   = 0.1
init_weight_decay = 0.0005

epochs         = 1 # _update_representation에 있음, 70이 오리지널
lrate          = 0.1
milestones     = [30, 50]
lrate_decay    = 0.1
batch_size     = 128
...
class WSC(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        # --------
        # 변경: BWT 계산용 test_loader도 추가
        self.bwt_test_loader = []
        task_class_ranges = [
            (0, 50),    # task 0: 0~49
            (50, 55),   # task 1: 50~54
            (55, 60),   # task 2: 55~59
            ...
            (85, 90),   # task 8: 85~89
            (90, 95),   # task 9: 90~94
            (95, 100),  # task 10: 95~99
        ]
        for start, end in task_class_ranges:
            bwt_test_dataset = data_manager.get_dataset(
                np.arange(start, end),
                source="test", mode="test"
            )
            bwt_loader = DataLoader(
                bwt_test_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers
            )
            self.bwt_test_loader.append(bwt_loader)

        print(f"########### DEBUG : incremental_train on Task {self._cur_task}")
        print("Train target classes:", np.arange(self._known_classes, self._total_classes))
        print("Test target classes:", np.arange(0, self._total_classes))
        train_labels = set()
        for i in range(len(train_dataset)):
            _, _, y = train_dataset[i]
            train_labels.add(int(y))
        test_labels = set()
        for i in range(len(test_dataset)):
            _, _, y = test_dataset[i]
            test_labels.add(int(y))
        print(f"[Task {self._cur_task}] Train unique labels: {sorted(train_labels)}")
        print(f"[Task {self._cur_task}] Test unique labels: {sorted(test_labels)}")
        # --------
    ...
    # ------------------------------------------------------------------
    # Not used in the paper, but kept for reference.
    # ------------------------------------------------------------------
    ...
    def _compute_moment_score(
        ...
        for p, g in zip(params, grads):
            ...
            # bias-correction – cheap, so keep it for reference. (optional) 
            step = st.get("t", 0) + 1
            st["t"] = step

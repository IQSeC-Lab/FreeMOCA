...
class AutoEncoder(Replayer):
    ...
    def __init__(self, image_size, image_channels, classes,
        ...
        #               hid_size=None, drop=0.5, size_per_layer = sizes_layers,
        #               batch_norm=fc_bn, nl=fc_nl, bias=bias,
        #               excitability=excitability, excit_buffer=excit_buffer, gated=gated)
        
        self.fcE = MLP(input_size=image_channels*image_size**2, output_size=fc_units, layers=fc_layers-1,
                       hid_size=fc_units, drop=self.fc_drop, size_per_layer = sizes_layers, batch_norm=fc_bn, nl=fc_nl, gated=gated)
        ...
        #MARK: sizes_layers_decoder
        sizes_layers_decoder = [128, 256, 512, 1024, 2401]
        # sizes_layers_decoder = [128, 256, 512, 1024, 2381]
        ##>----Decoder (= p[x|z])----<##
        # -from z
        ...
        self.fcD = MLP(input_size=fc_units, output_size=image_channels*image_size**2, layers=fc_layers-1,
                       hid_size=fc_units, drop=self.fc_drop, size_per_layer = sizes_layers_decoder, batch_norm=fc_bn, nl=fc_nl, gated=gated, output='BCE')
        # -to image-shape

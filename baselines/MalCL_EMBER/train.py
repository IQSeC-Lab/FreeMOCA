...
def data_task(config, X_train, Y_train, X_test, Y_test, scaler):
  ...
  X_test_bwt_t, Y_test_bwt_t = get_iter_bwt_test_dataset(X_test, Y_test, 
                                                n_class=config.n_class, n_inc=config.n_inc, task=config.task)
  acc_test_loader, _ = get_dataloader(X_test_acc_t, Y_test_acc_t, batchsize=config.batchsize, n_class=config.n_class, scaler = scaler, train=False)
  bwt_test_loader, _ = get_dataloader(X_test_bwt_t, Y_test_bwt_t, batchsize=config.batchsize, n_class=config.n_class, scaler = scaler, train=False)
  return X_train_t, Y_train_t, X_test_acc_t, Y_test_acc_t, X_test_bwt_t, Y_test_bwt_t, train_loader, acc_test_loader, bwt_test_loader, scaler

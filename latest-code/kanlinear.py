...
class KANLinear(torch.nn.Module):
    ...
    #     # assert x.dim() == 3 and x.size(2) == self.in_features
    #     # Handle 2D input by reshaping to 3D
    #      if x.dim() == 2:
    #         # [batch, in_features] -> [1, batch, in_features]
    #         x = x.unsqueeze(0)
    ...
    #     if output.size(0) == 1:
    #         return output.squeeze(0)  # [batch, out_features]

    # def forward(self, x: torch.Tensor) -> torch.Tensor:
    #     # Handle 2D input by reshaping to 3D
    #     if x.dim() == 2:
    #         x = x.unsqueeze(0)  # [batch, in_features] -> [1, batch, in_features]
    ...
    #     spline_output = torch.bmm(
    #         self.b_splines(x).view(x.size(0), x.size(1), -1),
    #         self.scaled_spline_weight.view(x.size(0), self.out_features, -1).permute(0, 2, 1),
    #     )

    #     output = base_output + spline_output

    #     # If input was originally 2D, return a 2D output
    #     if x.dim() == 2:
    #         return output.squeeze(0)  # [batch, out_features]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() == 2:
            input_was_2d = True
            if self.groups != 1:
                raise ValueError("2D input only supported with groups=1")
            x = x.unsqueeze(0)  # [1, batch, in_features]

        assert x.dim() == 3 and x.size(2) == self.in_features, \
            f"Expected [groups, batch, in_features], got {x.shape}"
        ...
        if input_was_2d:
            output = output.squeeze(0)  # Back to [batch, out_features]
